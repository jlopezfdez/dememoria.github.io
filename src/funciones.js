$(
    function () {
        ...
        g_combinaciones = new Object();//SIN var o let, porque esta function es closure y para el objeto global da problemas y se define pero luego se perdía la referencia.
        const TIEMPO_VISUALIZACION = 1000, ROTACION_FIN_PARTIDA = 3;
        const [JUEGO_INICIAL, TABLA_INICIAL] = ['La Liga', '3 parejas'];
       ...
       */
        ...
        */
                ...
                if (g_totalClicks == 2) {
                    ...
                    if (g_comparacion1 == g_comparacion2) {
                        ...
                        if (g_aciertosTotales == g_aciertosNecesarios) {
                            ...
                            var tiempoSegundos = parseInt(minutos * 60) + parseInt(segundos) + '.' + parseInt(centesimas);
        ...
        function comenzarJuego(params) {
            ...
            establecerMejorIntento(tipoJuegoActivo, tablaActiva, g_combinaciones[tipoJuegoActivo + tablaActiva].mejorIntento);
            establecerMejorTiempo(tipoJuegoActivo, tablaActiva, g_combinaciones[tipoJuegoActivo + tablaActiva].mejorTiempo);
        ...
        function registrarJuegosDisponibles() {
            ...
            arrayJuegos.forEach((element, index) => {
                ...
                g_tipos[element].formatos_disponibles.forEach((element2, index2) => {
                    ...
                    var objJuego = new Object();

                    objJuego[element + element2] = {};
                    objJuego[element + element2].intentos = new Set();
                    objJuego[element + element2].tiempos = new Set();
                    objJuego[element + element2].mejorIntento = 999;
                    objJuego[element + element2].mejorTiempo = 999;

                    Object.assign(g_combinaciones, objJuego);
                });
            });
        ...
        function establecerMejorIntento(tipoJuego, tipoTabla, numIntentos) {
            let mejorActual = g_combinaciones[tipoJuego + tipoTabla].mejorIntento;

            if (numIntentos != 999) {
                if (numIntentos < mejorActual) {
                    menor = numIntentos;
                    g_combinaciones[tipoJuego + tipoTabla].mejorIntento = menor;
                }
                else {
                    menor = mejorActual;
                }
        ...
        function establecerMejorTiempo(tipoJuego, tipoTabla, tiempoEmpleado) {
            let mejorActual = g_combinaciones[tipoJuego + tipoTabla].mejorTiempo;

            if (tiempoEmpleado != 999) {
                if (tiempoEmpleado < mejorActual) {
                    menor = tiempoEmpleado;
                    g_combinaciones[tipoJuego + tipoTabla].mejorTiempo = menor;
                }
                else {
                    menor = mejorActual;
                }
        ...
        function main() {
            ...
            g_tipoJuegoActivo = JUEGO_INICIAL;
            g_tipoTablaActiva = TABLA_INICIAL;
